...
class RAGEvaluator:
    ...
    async def generate_rag_response(
        ...
        try:
            ...
            return {
                "answer": answer,
                "contexts": contexts,  # full chunk text — only for RAGAS, not persisted
                "context_refs": context_refs,  # persisted: which references were retrieved
            }
            ...
            async with eval_semaphore:
                ...
                try:
                    ...
                    result = {
                        ...
                        "answer": rag_response["answer"],
                        "ground_truth": ground_truth,
                        "context_refs": rag_response.get("context_refs", []),
                        ...
                        "metrics": {
                            ...
                            "answer_correctness": float(
                                scores_row.get("answer_correctness", 0)
                            ),
    ...
    def _backfill_dataset(self, results: List[Dict[str, Any]]) -> None:
        """Backfill answer + context_refs into the dataset file in place.

        After evaluation, each test_case in the dataset JSON gains:
          - answer: the full RAG-generated answer
          - context_refs: which references (file + chunk count) were retrieved
        contexts full text is deliberately NOT written back (too large; re-query
        ...
        try:
            ...
            logger.warning(
                "Could not backfill dataset (%s): %s",
        ...
        # results are 1-indexed by test_number; test_cases are 0-indexed in order
        result_by_number = {
            r["test_number"]: r for r in results if "test_number" in r
        }
        backfilled = 0
        for i, tc in enumerate(test_cases):
            r = result_by_number.get(i + 1)
            if not r:
                continue
            tc["answer"] = r.get("answer", "")
            tc["context_refs"] = r.get("context_refs", [])
            backfilled += 1

        with open(self.test_dataset_path, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=2, ensure_ascii=False)
        logger.info(
            "✅ Backfilled answer + context_refs for %d/%d cases into %s",
            backfilled,
            len(test_cases),
            self.test_dataset_path,
        )
    ...
    def _display_results_table(self, results: List[Dict[str, Any]]):
        ...
        for result in results:
            ...
            if metrics:
                ...
                ans_corr = metrics.get("answer_correctness", 0)
    ...
    def _calculate_benchmark_stats(
        ...
        for result in valid_results:
            ...
            if not _is_nan(answer_correctness):
                metrics_data["answer_correctness"]["sum"] += answer_correctness
                metrics_data["answer_correctness"]["count"] += 1
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        # Backfill answer + context_refs into the dataset file (in place)
        self._backfill_dataset(results)
        ...
        logger.info("Average Answer Correctness:%.4f", avg["answer_correctness"])
